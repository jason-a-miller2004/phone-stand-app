...
export function unmute(context:AudioContext, allowBackgroundPlayback:boolean = false, forceIOSBehavior:boolean = false):{ dispose:{ ():void; }}
{
	...
	function addEventListeners(target:EventTarget, events:string[], handler:EventListenerOrEventListenerObject, capture:boolean, passive:boolean):void { for (let i:number = 0; i < events.length; ++i) target.addEventListener(events[i], handler, { capture: capture, passive: passive }); }
	function removeEventListeners(target:EventTarget, events:string[], handler:EventListenerOrEventListenerObject, capture:boolean, passive:boolean):void { for (let i:number = 0; i < events.length; ++i) target.removeEventListener(events[i], handler, <any>{ capture: capture, passive: passive }); }
